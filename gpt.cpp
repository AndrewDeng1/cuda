...
public:
    ...
    void register_params() override {
        register_param("key.weight", &key_.weight_);
        register_param("query.weight", &query_.weight_);
        register_param("value.weight", &value_.weight_);
    }
...
public:
    ...
    void register_params() override {
        register_param("proj.weight", &proj_.weight_);
        if(proj_.use_bias_) {
            register_param("proj.bias", &proj_.bias_);
        }
    }
...
public:
    ...
    pair<Tensor, Tensor> forward_with_targets(const Tensor& idx, const Tensor* targets = nullptr) {
        ...
        vector<float> pos_indices_vec;
        for(int i = 0; i < T; i++) {
            pos_indices_vec.push_back((float)i);
        }
        Tensor pos_indices_tensor({T}, pos_indices_vec, false);
...
pair<Tensor, Tensor> get_batch(const string& split) {
    ...
    uniform_int_distribution<> dis(0, data.size() - block_size - 1);
