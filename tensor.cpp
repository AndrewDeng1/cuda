...
std::shared_ptr<Tensor> Tensor::reshape(const std::vector<int>& new_shape) {
    ...
    assert(new_numel == this->numel());  // shapes must be compatible

    auto reshaped = std::make_shared<Tensor>(*this);  // shallow copy
...
shared_ptr<Tensor> Tensor::sum(int axis, bool keepdims) {
    ...
    // Convert rec to a lambda function
...
shared_ptr<Tensor> reduce_to_shape(shared_ptr<Tensor> A, vector<int>& target_shape) {
    ...
    vector<int> new_shape;
    if(target_shape.size() < A->shape.size()) {
        for(int i=0; i<A->shape.size()-target_shape.size(); i++) {
            new_shape.push_back(1);
        }
    }
    ...
    shared_ptr<Tensor> result = A;
    for(int i=0; i<new_shape.size(); i++) {
        if(new_shape[i]==1 && A->shape[i]>1) {
...
shared_ptr<Tensor> operator+(const shared_ptr<Tensor>& A, const shared_ptr<Tensor>& B) {
    ...
    result->backward_fn = [A, B, result](){
        if(A->requires_grad && A->grad!=nullptr){
            A->grad += reduce_to_shape(result->grad, A->shape);
        }
        if(B->requires_grad && B->grad!=nullptr){
            B->grad += reduce_to_shape(result->grad, B->shape);
        }
    };
    return result;
}
